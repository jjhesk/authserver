...
if (!class_exists('gformBase')) {
    class gformBase
    {
        ...
        protected static function gf_get_form_entries($form_id)
        {
            ...
            $listed = $wpdb->get_results("SELECT * FROM $table WHERE form_id=" . $form_id . " AND status='active' ORDER BY date_created DESC");
            return $listed;
        }
        ...
        protected static function gf_update_field_value($form_id, $lead_id, $field_id, $value_replacement)
        {
            ...
            $sql = "SELECT * FROM $table WHERE id=%d AND form_id=%d AND field_number=%d";
            ...
            if (!$result) {
                ...
                $rs = $wpdb->update($table, $insert, array('id' => $result->id));
            }
            return $rs;
        }
