...
class app_transaction_history
{
     ...
     * @return mixed
     */
    public static function get_history_api($uuid, $Q)
    {
        try {
            ...
            if (!isset($Q->sort)) $sort = "ref_code"; else $sort = $Q->sort;
            if (!isset($Q->order)) $order = "ase"; else $order = $Q->order;
            if (!isset($Q->transid)) $transid = ""; else $transid = $Q->transid;
            ...
            //   }
            $json = api_handler::curl_get(VCOIN_SERVER . "/api/account/coinhistory", $param, array(CURLOPT_TIMEOUT => 20));
            $R = json_decode($json);
            unset($json);
